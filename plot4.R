...
png(file="plot4.png")
par(mfcol=c(2,2))
plot(strptime(paste(sdf$Date, sdf$Time), "%d/%m/%Y %H:%M:%S"), as.numeric(levels(sdf$Global_active_power))[sdf$Global_active_power], type="l", ylab="Global Active Power", xlab="")
...
plot(strptime(paste(sdf$Date, sdf$Time), "%d/%m/%Y %H:%M:%S"), as.numeric(levels(sdf$Voltage))[sdf$Voltage], type="l", ylab="Voltage", xlab="datetime")
plot(strptime(paste(sdf$Date, sdf$Time), "%d/%m/%Y %H:%M:%S"), as.numeric(levels(sdf$Global_reactive_power))[sdf$Global_reactive_power], type="l", ylab="Global_reactive_power", xlab="datetime")
dev.off()